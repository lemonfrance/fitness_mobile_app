...
class FitBitService {
  ...
  Future getCode() async {
    ...
    final result = await FlutterWebAuth.authenticate(
        url: url,
        callbackUrlScheme: 'http://localhost');
  ...
  Future<http.Response> getAuthToken(String code) {
    return http.post(Uri.parse('https://api.fitbit.com/oauth2/token?code=$code&grant_type=authorization_code&redirect_uri=http://localhost'),
      headers: {
