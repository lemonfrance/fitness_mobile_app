...
class EvaluationService {
  ...
  Future setResponses(String date, String exerciseType, bool chestPain, bool coldSweats, int difficulty, int pain) async {
    return await evaluationCollection.doc(userId)
        .collection('completedWorkouts').doc(date).set({
  ...
  Future setHeartRateData(String date) async {
    return await evaluationCollection.doc(userId)
        .collection('completedWorkouts').doc(date).update({
          'heartRates': workoutHeartRatesDB
    });
  }
